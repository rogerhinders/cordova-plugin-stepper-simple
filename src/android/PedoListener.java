...
public class PedoListener extends CordovaPlugin implements SensorEventListener {
  ...
  private void getSteps(JSONArray args) {
    Log.d("STEPPER", "getSteps START");
    long date = 0;
    try {
      date = args.getLong(0);
    }
    catch (JSONException e) {
      Log.d("STEPPER", "getSteps exc 0");
      e.printStackTrace();
      return;
    }

    Database db = Database.getInstance(getActivity());
    int steps = db.getSteps(-1); //date);
    db.close();
    ...
    try {
      ...
      e.printStackTrace();
      Log.d("STEPPER", "getSteps exc 1");
      return;
    }
    Log.d("STEPPER", "getSteps END");
...
*/
  ...
  private void start(JSONArray args) throws JSONException {
    ...
    Log.d("STEPPER", "Starting stepper.");
  ...
  private void initSensor() {
    ...
    if ((status == PedoListener.RUNNING) || (status == PedoListener.STARTING)
      && status != PedoListener.PAUSED) {
      Log.d("STEPPER", "PedoListener already running");
      return;
    }

    Log.d("STEPPER", "Starting pedolistener");
  ...
  public void onSensorChanged(final SensorEvent event) {
    return;
    /*
    Log.d("STEPPER", "onSensorChanged!");
    if (status == PedoListener.STOPPED) {
      Log.d("STEPPER", "Sensor changed, but listener stopped");
      return;
    }
    status = PedoListener.RUNNING;

    Log.d("STEPPER", "sensor changed and listener runnin");
